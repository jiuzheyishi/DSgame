...
def masked_cross_entropy(class_dist, target, trg_mask, trg_lens=None,
    ...
    # [batch_size*trg_seq_len, num_classes]
    class_dist_flat = class_dist.view(-1, num_classes)
    ...
    loss = losses.sum(dim=1)  # [batch_size]
    if orthogonal_loss:
        orthogonal_loss = compute_orthogonal_loss(
            delimiter_hidden_states, delimiter_hidden_states_lens)  # [batch_size]
...
def compute_coverage_losses(coverage, attn_dist):
    ...
    coverage_flat = coverage.view(-1, src_seq_len)
    # [batch_size * trg_seq_len, src_seq_len]
    attn_dist_flat = attn_dist.view(-1, src_seq_len)
    coverage_losses_flat = torch.sum(
        torch.min(attn_dist_flat, coverage_flat), dim=1)  # [batch_size * trg_seq_len]
    coverage_losses = coverage_losses_flat.view(
        batch_size, trg_seq_len)  # [batch, trg_seq_len]
...
def masked_coverage_loss(coverage, attn_dist, trg_mask):
    ...
    attn_dist_flat = attn_dist.view(-1, src_seq_len)
    coverage_losses_flat = torch.sum(
        torch.min(attn_dist_flat, coverage_flat), 1)  # [batch_size * trg_seq_len]
    coverage_losses = coverage_losses_flat.view(
        *trg_mask.size())  # [batch, trg_seq_len]
    if trg_mask is not None:
...
def compute_orthogonal_loss(delimiter_hidden_states, delimiter_hidden_states_lens=None):
    ...
    orthogonal_loss = torch.norm(orthogonal_loss_.view(
        batch_size, -1), p=2, dim=1)  # [batch]
...
def loss_debug():
    ...
    decoder_size = 100
    num_delimiter = 5
    delimiter_hidden_states = torch.randn(
        batch_size, decoder_size, num_delimiter)
...
def compute_orthogonal_loss_debug():
    ...
    num_delimiter_1 = 5
    # [batch_size, decoder_size, num_delimiter]
    delimiter_hidden_states_1 = torch.randn(
        batch_size_1, decoder_size_1, num_delimiter_1)
    ...
    decoder_size_2 = 10
    num_delimiter_2 = 3
    delimiter_hidden_states_2 = torch.zeros(
        batch_size_2, decoder_size_2, num_delimiter_2)
    ...
    ortho_loss_2 = compute_orthogonal_loss(
        delimiter_hidden_states_2, delimiter_hidden_states_2_lens)
    # print(delimiter_hidden_states_2[0])
    print(ortho_loss_2)
    assert ortho_loss_2.size() == torch.Size([batch_size_2]) and ortho_loss_2[0].item(
    ) == 0.0 and math.fabs(ortho_loss_2[1].item() - math.sqrt(2)) < 1e-3

    batch_size_3 = 3
    decoder_size_3 = 10
    num_delimiter_3 = 4
    delimiter_hidden_states_3 = torch.zeros(
        batch_size_3, decoder_size_3, num_delimiter_3)
    ...
    ortho_loss_3 = compute_orthogonal_loss(
        delimiter_hidden_states_3, delimiter_hidden_states_3_lens)
    # print(delimiter_hidden_states_2[0])
    print(ortho_loss_3)
