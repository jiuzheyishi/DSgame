    ...
    for epoch in range(0, opt.epochs):
        ...
        for batch_i, batch in enumerate(train_data_loader):
            ...
            train_loss = train_one_batch(
                batch, overall_model, optimizer_ml, opt)
            ...
            if epoch >= 1:
                if total_batch % opt.checkpoint_interval == 0:
                    ...
                    valid_loss = evaluate_loss(
                        valid_data_loader, overall_model, opt)
                    ...
                    if epoch >= opt.start_decay_and_early_stop_at:
                        ...
                        if not opt.disable_early_stop:
                            ...
                            if num_stop_dropping >= opt.early_stop_tolerance:
                                print(
                                    'Have increased for %d check points, early stop training' % num_stop_dropping)
...
def train_one_batch(batch, overall_model, optimizer, opt):
    ...
    decoder_dist, h_t, seq2seq_attention_dist, encoder_final_state, coverage = \
        overall_model(src, src_lens, trg, src_oov, max_num_oov, src_mask,
                      trg_mask, src_sent_positions, src_sent_nums, src_sent_mask)
