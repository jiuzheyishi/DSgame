...
class RNNDecoder(nn.Module):
    def __init__(self, vocab_size, embed_size, hidden_size, num_layers, memory_bank_size, coverage_attn, copy_attn,
        ...
        self.vocab_dist_linear_1 = nn.Linear(
            hidden_size + merged_memory_bank_size, hidden_size)
    ...
    def forward(self, y, h, memory_banks, src_masks, max_num_oovs, src_oov, coverage, decoder_memory_bank=None, rating_memory_bank=None, goal_vector=None):
        ...
        word_context, word_attn_dist, coverage = self.attention_layer(
            last_layer_h_next, memory_bank, src_mask, coverage)
        # context: [batch_size, memory_bank_size], attn_dist: [batch_size, max_input_seq_len], coverage: [batch_size, max_input_seq_len]
        assert word_context.size() == torch.Size(
            [batch_size, self.memory_bank_size])
        assert word_attn_dist.size() == torch.Size(
        ...
        vocab_dist = self.softmax(self.vocab_dist_linear_2(
            self.dropout(self.vocab_dist_linear_1(vocab_dist_input))))
        ...
        if self.copy_attn:
            ...
            p_gen_input = torch.cat(
                (context, last_layer_h_next, y_emb.squeeze(0)), dim=1)
            ...
            assert final_dist.size() == torch.Size(
                [batch_size, self.vocab_size + max_num_oovs])
        else:
