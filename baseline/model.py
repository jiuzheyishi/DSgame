class Seq2SeqModel(nn.Module):
    ...
    def __init__(self, opt):
        ...
        if self.bridge == 'copy':
            assert self.encoder_size * \
                self.num_directions == self.decoder_size, 'encoder hidden size and decoder hidden size are not match, please use a bridge layer'
    ...
    def init_embedding_weights(self):
        ...
        if not self.share_embeddings:
            self.decoder.embedding.weight.data.uniform_(
    ...
    def forward(self, src, src_lens, trg, src_oov, max_num_oov, src_mask, trg_mask, src_sent_positions, src_sent_nums, src_sent_mask):
        ...
        # Encoding
        memory_banks, encoder_final_states = self.encoder(
            src, src_lens, src_mask, src_sent_positions, src_sent_nums)
        ...
        assert word_memory_bank.size() == torch.Size(
            [batch_size, max_src_len, self.num_directions * self.encoder_size])
        assert word_encoder_final_state.size() == torch.Size(
            [batch_size, self.num_directions * self.encoder_size])

        # Decoding
        # [dec_layers, batch_size, decoder_size]
        h_t_init = self.init_decoder_state(word_encoder_final_state)
        ...
        for t in range(max_target_length):
            ...
            decoder_dist, h_t_next, _, attn_dist, p_gen, coverage = \
                self.decoder(y_t, h_t, memory_banks, src_masks,
                             max_num_oov, src_oov, coverage, decoder_memory_bank)
            # [batch, 1, vocab_size]
            decoder_dist_all.append(decoder_dist.unsqueeze(1))
            # [batch, 1, src_seq_len]
        ...
        decoder_dist_all = torch.cat(decoder_dist_all, dim=1)
        ...
        if self.copy_attn:
            ...
            assert decoder_dist_all.size() == torch.Size(
                (batch_size, max_target_length, self.vocab_size))
        assert attention_dist_all.size() == torch.Size(
            (batch_size, max_target_length, max_src_len))
    ...
    def tensor_2dlist_to_tensor(self, tensor_2d_list, batch_size, hidden_size, seq_lens):
        ...
        for i in range(batch_size):
            for j in range(max_seq_len - seq_lens[i]):
                tensor_2d_list[i].append(torch.ones(hidden_size).to(
                    self.device) * self.pad_idx_trg)  # [hidden_size]
            tensor_2d_list[i] = torch.stack(
                tensor_2d_list[i], dim=1)  # [hidden_size, max_seq_len]
        # [batch_size, hidden_size, max_seq_len]
        tensor_3d = torch.stack(tensor_2d_list, dim=0)
        return tensor_3d
    ...
    def init_decoder_state(self, encoder_final_state):
        ...
        decoder_init_state = decoder_init_state.unsqueeze(0).expand(
            (self.dec_layers, batch_size, self.decoder_size))
        # [dec_layers, batch_size, decoder_size]
