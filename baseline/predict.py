...
def init_pretrained_model(pretrained_model_path, opt, rating_tokens_tensor):
    if opt.model_type == 'hss':
        ...
        overall_model = MultiTaskBasicClassifySeq2Seq(
...
def preprocess_beam_search_result(beam_search_result, idx2word, vocab_size, oov_lists, eos_idx, unk_idx, replace_unk, src_str_list):
    ...
    for pred_n_best, score_n_best, attn_n_best, dec_state_n_best, oov, src_word_list in zip(predictions, scores, attention, dec_states, oov_lists, src_str_list):
        ...
        for pred, attn in zip(pred_n_best, attn_n_best):
            sentence = prediction_to_sentence(
                pred, idx2word, vocab_size, oov, eos_idx, unk_idx, replace_unk, src_word_list, attn)
        ...
        # a list of FloatTensor[output sequence length, memory_bank_size], with len = [n_best]
        pred_dict['dec_states'] = dec_state_n_best
...
def preprocess_hss_beam_search_result(beam_search_result, idx2word, vocab_size, oov_lists, eos_idx, unk_idx, replace_unk, src_str_list):
    ...
    scores = beam_search_result['scores']
    attention = beam_search_result['attention']
    # a list of list, len=(batch, n_best), tensor = [out_seq_len, memory_bank_size], seq_len including eos
    sentiment_context = beam_search_result['sentiment_context']
    assert len(predictions) == batch_size
    pred_list = []  # a list of dict, with len = batch_size
    for pred_n_best, score_n_best, attn_n_best, senti_n_best, oov, src_word_list in zip(predictions, scores, attention, sentiment_context, oov_lists, src_str_list):
        ...
        pred_dict['attention'] = attn_n_best
        # a list of FloatTensor[output sequence length, memory_bank_size], with len = [n_best]
        pred_dict['sentiment_context'] = senti_n_best
        pred_list.append(pred_dict)
    return pred_list
...
def predict(test_data_loader, overall_model, opt):
    ...
    with torch.no_grad():
        for batch in tqdm(test_data_loader):
            ...
            if isinstance(overall_model, HSSModel):
                beam_search_result, encoder_memory_bank = generator.hss_beam_search(
                    src, src_lens, src_oov, src_mask, oov_lists, opt.word2idx, src_sent_positions, src_sent_nums, src_sent_mask)
                # a dictionary, with predictions, scores, attention and sentiment_context
                pred_list = preprocess_hss_beam_search_result(
                    beam_search_result, opt.idx2word, opt.vocab_size, oov_lists, io.EOS, io.UNK, opt.replace_unk, src_str_list)
                # pred_list: list of {"sentences": [], "scores": [], "attention": [], "sentiment_context": []}

                # construct the sentiment_context_tensor for the classifier
                sentiment_context_list = [
                    pred_result['sentiment_context'][0] for pred_result in pred_list]
                # get the output sequence lens with EOS
                out_lens = [sentiment_context.size(
                    0) for sentiment_context in sentiment_context_list]
                ...
                adding_src_mask = (
                    1 - expand_src_mask).masked_fill((1 - expand_src_mask).byte(), -float('inf'))
                ...
                adding_out_mask = (
                    1 - expand_out_mask).masked_fill((1 - expand_out_mask).byte(), -float('inf'))
                sentiment_context_tensor = sentiment_context_tensor * \
                    expand_out_mask + adding_out_mask

                # sentiment_context_tensor, encoder_memory_bank
                enc_logit = classifier_model(
                    encoder_memory_bank, sentiment_context_tensor)
            ...
            #         enc_logit = classifier_output
                ...
                beam_search_result, enc_logit = generator.word_attn_beam_search(
                    src, src_lens, src_oov, src_mask, oov_lists, opt.word2idx, src_sent_positions, src_sent_nums, src_sent_mask)
                pred_list = preprocess_beam_search_result(beam_search_result, opt.idx2word, opt.vocab_size, oov_lists,
                                                          io.EOS, io.UNK, opt.replace_unk, src_str_list)
                ...
                if overall_model.dec_classify_input_type == 'attn_vec':
                    ...
                    sentiment_context_tensor = torch.nn.utils.rnn.pad_sequence(
                        sentiment_context_list, batch_first=True)  # [batch, max_out_seq_len, memory_bank_size]
                    ...
                    dec_states_list = [pred_result['dec_states'][0]
                                       for pred_result in pred_list]
                    # get the output sequence lens with EOS
                    out_lens = [dec_state.size(0)
                                for dec_state in dec_states_list]

                    # list of [output sequence length, memory_bank], len=batch, padding
                    dec_states_tensor = torch.nn.utils.rnn.pad_sequence(
                        dec_states_list, batch_first=True)  # [batch, max_out_seq_len, memory_bank_size]
                ...
                out_mask = out_mask.cuda()
                # sentiment_context_tensor, encoder_memory_bank
                dec_logit = overall_model.dec_classifier(
                    dec_classifier_input, out_mask)
                ...
                _, _, _, _, _, enc_logit, classifier_attention_dist = \
                    overall_model(src, src_lens, None, src_oov, None, src_mask,
                                  None, None, src_sent_positions, src_sent_nums, src_sent_mask)
                pred_list = None
                ...
                if enc_rating_preds is None:
                    if opt.ordinal:
                        enc_rating_preds = binary_results_to_rating_preds(
                            enc_logit.detach().cpu().numpy()) if enc_logit is not None else None
                        dec_rating_preds = binary_results_to_rating_preds(
                            dec_logit.detach().cpu().numpy()) if dec_logit is not None else None
                        # add for merged rating preds
                        if dec_logit is not None:
                            # [batch, 5]
                            merged_logit = (enc_logit + dec_logit) / 2
                            merged_rating_preds = binary_results_to_rating_preds(
                                merged_logit.detach().cpu().numpy())
                    else:
                        enc_rating_preds = enc_logit.detach().cpu(
                        ).numpy() if enc_logit is not None else None
                        dec_rating_preds = dec_logit.detach().cpu(
                        ).numpy() if dec_logit is not None else None
                        # add for merged rating preds
                        if dec_logit is not None:
                            # [batch, 5]
                            merged_logit = (enc_logit + dec_logit) / 2
                    ...
                    if opt.ordinal:
                        ...
                        enc_rating_preds = np.append(enc_rating_preds, enc_logit.detach(
                        ).cpu().numpy(), axis=0) if enc_rating_preds is not None else None
                        dec_rating_preds = np.append(dec_rating_preds, dec_logit.detach(
                        ).cpu().numpy(), axis=0) if dec_rating_preds is not None else None
                        if dec_logit is not None:
                            # [batch, 5]
                            merged_logit = (enc_logit + dec_logit) / 2
                            merged_rating_preds = np.append(
                                merged_rating_preds, merged_logit.detach().cpu().numpy(), axis=0)
            else:
                # append rating outputs
                if enc_rating_preds is None:
                    ...
                    enc_rating_preds = np.append(
                        enc_rating_preds, batch_rating_tensor.detach().cpu().numpy(), axis=0)
            ...
            if pred_list is not None:
                for src_str, pred, oov in zip(src_str_list, pred_list, oov_lists):
                    ...
                    # predicted sentences from a single src, a list of list of word, with len=[n_best, out_seq_len], does not include the final <EOS>
                    ...
                    decode_out_sent_list = nltk.tokenize.sent_tokenize(
                        decode_out_str)
    ...
    if not isinstance(overall_model, ExternalFeedClassifySeq2Seq) and not opt.ordinal:
        ...
        merged_rating_preds = np.argmax(
            merged_rating_preds, axis=1) if merged_rating_preds is not None else None
...
def main(opt):
    ...
    if os.path.exists(os.path.join(model_dir_path, 'rating_tokens_tensor.pt')):
        rating_tokens_tensor = torch.load(os.path.join(
            model_dir_path, 'rating_tokens_tensor.pt'))
    else:
    ...
    overall_model = init_pretrained_model(
        opt.pretrained_model, old_opt, rating_tokens_tensor)
    ...
    if opt.teacher_force_evaluate:
        ...
        evaluate_loss(test_loader, overall_model,
                      val_classification_loss_func, old_opt, print_incon_stats=True)
    else:
